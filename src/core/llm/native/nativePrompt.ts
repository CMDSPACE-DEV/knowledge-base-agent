...
export function buildNativePrompt(messages: RequestMessage[]): {
  ...
  return {
    systemPrompt: [
      systemPrompt,
      'You are running inside CMDS Achmage. Do not read, write, search, or execute files and commands directly. Use only the CMDS Achmage tools supplied for this request. Return the requested answer in Markdown.',
    ]
      .filter(Boolean)
      .join('\n\n'),
    prompt: [
